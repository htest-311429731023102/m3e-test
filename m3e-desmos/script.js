...
(async () => {
	const {argbFromHex, hexFromArgb, themeFromSourceColor, redFromArgb, greenFromArgb, blueFromArgb} = await import("https://esm.sh/@material/material-color-utilities?exports=argbFromHex,themeFromSourceColor,redFromArgb,greenFromArgb,blueFromArgb")

const getMissingColorMap = (
    theme,
    mode
) => {
    ...
    const lightColors = new Map([
        ...
        ['--md-sys-color-surface-container-lowest', rgbFromArgb(neutral.tone(100))],
...
missing.forEach((color, token) => document.documentElement.style.setProperty(token, color));
}
...
try {
    if (orig) Calc.controller.grapher2d.gridLayer.redrawToCtx = orig;
} catch (e) {}
const orig = Calc.controller.grapher2d.gridLayer.redrawToCtx.bind(Calc.controller.grapher2d.gridLayer);
Calc.controller.grapher2d.gridLayer.redrawToCtx = ((...args) => {
    const ctx = Calc.controller.grapher2d.canvasLayer.ctx;
    const origStroke = ctx.stroke.bind(ctx);
    const computed = getComputedStyle(ctx.canvas);
    const col = computed.color.replaceAll('rgb(', '').replaceAll('rgba(', '').replaceAll(')', '').split(', ').slice(0, 3);
    const c = "#" + col.map(v => (parseInt(v)).toString(16).padStart(2, "0")).join("");
    Calc.controller.grapher2d.settings.config.setProperty("backgroundColor", computed.backgroundColor);
    ...
    return r;
}).bind(Calc.controller.grapher2d.gridLayer);
	} catch(e) {}
}, 10);
